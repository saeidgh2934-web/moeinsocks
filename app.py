...
st.subheader("۱. آپلود عکس مرجع (بهینه‌شده برای گوشی و تصاویر سنگین)")
uploaded_file = st.file_uploader("انتخاب تصویر مرجع محصول از گالری", type=["jpg", "jpeg", "png", "webp", "heic", "heif"])
...
if uploaded_file is not None:
    ...
    try:
        ...
        if bytes_data:
            ...
            # Prevent memory overflow / server crash on high-res mobile photos by resizing max dim to 1200px
            raw_image.thumbnail((1200, 1200), Image.Resampling.LANCZOS)
            image = raw_image
    ...
    if image is not None:
        ...
        with col1:
            st.image(image, caption="تصویر مرجع آپلودشده (بهینه‌شده)", use_container_width=True)
            
        with col2:
            st.subheader("۲. خروجی تحلیل خودکار شورا")
            st.success("تصویر با موفقیت دریافت و بارگذاری شد!")
